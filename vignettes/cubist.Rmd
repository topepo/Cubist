...
```{r bh4}
set.seed(1)
com_model <- cubist(x = train_pred, y = train_resp,
                         committees = 5)
summary(com_model)
``` 
...
[`caret](http://cran.r-project.org/web/packages/caret/index.html) package can be used to optimize these parameters. For example:
