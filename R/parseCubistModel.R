...
getSplits <- function(x) {
  ...
  splitData$type <- ""
  if (any(isType2)) {
    splitData$type[isType2] <- "type2"
  }
  if (any(isType3)) {
    splitData$type[isType3] <- "type3"
  }
...
printCubistRules <- function(x, dig = max(3, getOption("digits") - 5)) {
  ...
  cat(
    "Number of rules per committees:",
  ...
  isType2 <- grepl("^type=\"2\"", x)
  if (any(isType2)) {
    cond[isType2] <- type2(x[isType2], dig = dig)$text
  }
  isType3 <- grepl("^type=\"3\"", x)
  if (any(isType3)) {
    cond[isType3] <- type3(x[isType3])$text
  }
...
coef.cubist <- function(object, varNames = NULL, ...) {
  ...
  isEqn <- grepl("^coeff=", x)
  coefs <-
    eqn(x[isEqn], dig = 0, text = FALSE, varNames = varNames)
  p <- length(coefs)
